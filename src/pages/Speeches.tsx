      ...
      <div className="max-w-4xl mx-auto px-6 py-16">
        ...
        <div className="mb-12">
          <h1 className="text-4xl md:text-5xl font-bold text-black mb-4 flex items-center gap-3">
            ...
            {t('speechesPage.title')}
          </h1>
          <p className="text-lg text-gray-600">
            {t('speechesPage.subtitle')}
          </p>
        </div>
          ...
          {sortedSpeeches.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-600">
                {t('speechesPage.empty')}
              </p>
            </div>
          ) : (
            sortedSpeeches.map((speech) => {
              const title = language === 'pt' ? speech.title : speech.title_en || speech.title;
              const event = language === 'pt' ? speech.event : speech.event_en || speech.event;
              const description =
                language === 'pt' ? speech.description : speech.description_en || speech.description;
                  ...
                  <div className={`${speech.imageUrl ? 'md:col-span-2' : 'md:col-span-1'} p-6 flex flex-col justify-between`}>
                    <div>
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <h2 className="text-2xl font-bold text-black flex-1">{title}</h2>
                      ...
                      <p className="text-sm font-semibold text-pink-600 mb-3">{event}</p>
